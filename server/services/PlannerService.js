...
2. **技术方案**：给出技术栈选型(+理由)、架构分层、数据模型(schema/字段)、关键 API 设计、目录结构。
   **选型铁律——用户未明确指定技术时，优先选当下最先进、最前沿的技术，而非保守的主流方案**：
   - 编程语言：系统/CLI/后端/高性能/底层/网络/区块链/嵌入式等场景**首选 Rust**；其余场景选该领域当下最先进的语言（如智能合约选最新 Solidity、数据/AI 选最新 Python 生态、需要极致性能/安全优先 Rust）。
   - 前端/全栈：选最新最先进的框架与工具链（如最新版 Next.js/React、Vite、TypeScript、现代构建链），不要因循守旧。
   - 仍需保证场景可行性与生态可落地（如纯浏览器交互不强行 Rust(WASM)，但同等可行下永远偏向更先进者）；用户若已明确指定技术栈则尊重用户。

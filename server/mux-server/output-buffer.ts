...
export class OutputBufferManager {
   ...
   * 获取或创建会话的输出缓冲
   */
  getBuffer(sessionId: string): OutputBuffer {
   ...
   * 追加数据到会话缓冲
   */
  append(sessionId: string, data: Buffer | string): void {
   ...
   * 获取会话的输出历史
   */
  getHistory(sessionId: string): Buffer {
   ...
   * 获取会话的输出历史（Base64 编码）
   */
  getHistoryBase64(sessionId: string): string {
    const buffer = this.buffers.get(sessionId);
   ...
   * 删除会话的缓冲
   */
  deleteBuffer(sessionId: string): boolean {
